...
Ext.define("PMDMeta.view.main.Main", {
  ...
  initComponent: function() {
      ...
      handler: function() {
        ...
        Ext.Ajax.request({
          ...
          success: function(response, opts) {
            ...
            dt.setTime(Date.now());
            var filename =
              title + "_" + Ext.Date.format(dt, "YmdHi") + "_datacite.xml";
      ...
      handler: function() {
        ...
        Ext.Ajax.request({
          ...
          success: function(response, opts) {
            ...
            dt.setTime(Date.now());
            var filename =
              title + "_" + Ext.Date.format(dt, "YmdHi") + "_iso19139.xml";
    ...
    var datapreview = new Ext.Action({
      ...
      handler: function() {
        ...
        var url;
        if (href.indexOf("?") > 0) url = href.substr(0, href.indexOf("?"));
        else url = href;
        ...
        Ext.Ajax.request({
          ...
          success: function(response, opts) {
            ...
            if (
              ...
              responseData.versionstatus === "released"
            ) {
              ispublished = true;
            }
            ...
            if (ispublished) {
              ...
              Ext.Msg.show({
                ...
                message:
                  "Dataset is not public.<br>You can set the dataset status via menu <b>Publication</b> &gt; <b>Status</b>",
                buttons: Ext.Msg.OK,
                icon: Ext.Msg.INFO,
                fn: function(btn) {
                  me.doiregistration.show();
    ...
    me.datasetstatus = function(operation, errormsg, waitmsg) {
      ...
      Ext.Ajax.request({
        url:
          "resources/publishstatus.php?id=" +
          itemid +
          "&operation=" +
          operation,
        success: function(response, opts) {
    ...
    var pendinginrevision = new Ext.Action({
      ...
      handler: function() {
        ...
        var errormsg =
          "Could not transfer dataset to status &quot;" + status + "&quot;.";
        var waitmsg =
          "Transferring dataset to status &quot;" + status + "&quot;.";
      ...
      tbar: [
        {
          ...
          menu: [
            ...
            {
              text: "Export",
              menu: [metasavedatacite, metasaveiso]
            }
          ]
        },
        {
        ...
        },
        {
          ...
          menu: [
            {
              text: "Status",
              menu: [pendinginrevision, submit, publish, withdraw],
              listeners: {
                      ...
                      if (responseData.success) {
                        switch (responseData.versionstatus) {
                          ...
                          case "in-revision":
                            ...
                            publish.disable();
                            withdraw.disable();
                            pendinginrevision.setText(
                              ">" + pendinginrevision.getText().replace(/>/, "")
                            );
                            submit.setText(submit.getText().replace(/>/, ""));
                            publish.setText(publish.getText().replace(/>/, ""));
                            withdraw.setText(
                              withdraw.getText().replace(/>/, "")
                            );
                            break;
                          case "submitted":
                            if (
                              responseData.publicstatus === "pending" ||
                              responseData.publicstatus === "in-revision" ||
                              responseData.publicstatus === "submitted"
                            )
                              pendinginrevision.enable();
                            else pendinginrevision.disable();
                            submit.disable();
                            publish.enable();
                            withdraw.disable();
                            pendinginrevision.setText(
  ...
  items: [
    ...
    { region: "east", xtype: "FileAndVersion-Form" }
  ]
});
