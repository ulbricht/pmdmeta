...
Ext.define("PMDMeta.Application", {
  ...
  requires: ["PMDMeta.store.escidoc.Item"],
  name: "PMDMeta",
  ...
  launch: function() {
    ...
    if (urlparameter.formview === "bib") {
      ...
      Ext.each(Ext.ComponentQuery.query("DataCite-Rights"), function(
        elem,
        idx
      ) {
        elem.columns[1].hide();
      });
      Ext.each(Ext.ComponentQuery.query("DataCite-Authors"), function(
        elem,
        idx
      ) {
        elem.columns[7].hide();
      ...
      Ext.each(Ext.ComponentQuery.query("DataCite-Contributors"), function(
        elem,
        idx
      ) {
        elem.columns[7].hide();
      ...
      Ext.each(Ext.ComponentQuery.query("DataCite-Resource"), function(
        elem,
        idx
      ) {
        elem.columns[2].hide();
      ...
      Ext.each(Ext.ComponentQuery.query("panel #tbarhide1"), function(
        elem,
        idx
      ) {
        elem.setHidden(true);
      });
      Ext.each(Ext.ComponentQuery.query("panel #tbarhide2"), function(
        elem,
        idx
      ) {
        elem.setHidden(true);
      });
      Ext.each(Ext.ComponentQuery.query("panel #tbarhide3"), function(
        elem,
        idx
      ) {
        elem.setHidden(true);
    ...
    if (urlparameter.action === "new") {
      ...
      if (urlparameter.action === "template") {
        itemstore.loadtemplatedata({
          id: null,
          href: urlparameter.object,
          local: null
        });
      } else {
        itemstore.loaddata({
    ...
    Ext.apply(Ext.form.field.VTypes, {
      ...
      ARKText:
        "An ark link has to begin with <b>http://NMA/ark:</b> or <b>ark:</b>.",
    ...
    Ext.apply(Ext.form.field.VTypes, {
      ...
      bibcodeText:
        "The first characters of a bibcode should be a for digit year.",
    ...
    Ext.apply(Ext.form.field.VTypes, {
      ...
      ISBNText:
        "Enter ISBN10 or ISBN13 number. The position of hyphens does not matter.",
      ISBNMask: /[0-9\-]/i
    });
    Ext.apply(Ext.form.field.VTypes, {
      ISSN: function(v) {
    ...
    Ext.apply(Ext.form.field.VTypes, {
      ...
      floatvalText:
        "Specify signed numbers in the WGS84 system. Preferably specify 5 decimal places after the decimal point.",
      floatvalMask: /[0-9\.\-]/i
    });
    Ext.apply(Ext.form.field.VTypes, {
      intval: function(v) {
